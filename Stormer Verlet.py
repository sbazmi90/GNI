...
# # Stormer Verlet for solar system
...
for t in range(1,N):
    ...
    for l in range(10):
            px[t,l] = px[t-1,l] + 0.5*delta*ux[l]
            py[t,l] = py[t-1,l] + 0.5*delta*uy[l]
            pz[t,l] = pz[t-1,l] + 0.5*delta*uz[l]
    for l in range(10):
            x[t,l] = x[t-1,l] + delta*px[t,l]/m[l]
            y[t,l] = y[t-1,l] + delta*py[t,l]/m[l]
            z[t,l] = z[t-1,l] + delta*pz[t,l]/m[l]
    for k in range(10):
        ux[k] = uy[k] = uz[k] = 0
    for i in range(10):
        for j in range(10):
            if(i != j):
                ...
                uz[i] = uz[i]-G*m[i]*m[j]*dz[i,j]/((r[i,j]+ep)**3)
    for l in range(10):
        px[t,l] = px[t,l] + 0.5*delta*ux[l]
        py[t,l] = py[t,l] + 0.5*delta*uy[l]
        pz[t,l] = pz[t,l] + 0.5*delta*uz[l]
...
time = np.arange(N-1)
H_Stormer_Verlet = np.zeros((N-1),float)




for i in range(0,N-1):
    H_Stormer_Verlet[i] = UE[i]/2 + KE[i]




datafile_path = "./H-Stormer_Verlet.txt"
with open(datafile_path, 'w+') as datafile_id:
    np.savetxt(datafile_id, H_Stormer_Verlet)
...
plt.plot(time,H_Stormer_Verlet)
plt.xlim(0,365000)
plt.xlabel("Time",fontsize = 18)
plt.ylabel("Energy", fontsize = 18)
plt.title("Energy for Stormer_Verlet method",fontsize = 18)



# ### 3D plots for planets
...
ax.set_title("Part of Solar System by Stormer Verlet",fontsize = 25)
